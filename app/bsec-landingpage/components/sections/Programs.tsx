...
type TabType = keyof typeof programsData;
      ...
      <div className="max-w-[1280px] mx-auto px-6">
        <div className="flex flex-col md:flex-row justify-between items-end mb-12 gap-6">
          ...
          <div className="flex gap-4 overflow-x-auto pb-2 border-b border-outline-variant w-full md:w-auto scrollbar-hide">
            <button 
              className={`px-6 py-2 font-semibold text-sm transition-all whitespace-nowrap ${activeTab === 'sd' ? 'active-tab' : 'text-secondary'}`}
              onClick={() => setActiveTab('sd')}
            >
              Program SD
            </button>
            <button 
              className={`px-6 py-2 font-semibold text-sm transition-all whitespace-nowrap ${activeTab === 'smp' ? 'active-tab' : 'text-secondary'}`}
              onClick={() => setActiveTab('smp')}
            >
              Program SMP
            </button>
            <button 
              className={`px-6 py-2 font-semibold text-sm transition-all whitespace-nowrap ${activeTab === 'sma' ? 'active-tab' : 'text-secondary'}`}
              onClick={() => setActiveTab('sma')}
            >
              Program SMA
            </button>
            <button 
              className={`px-6 py-2 font-semibold text-sm transition-all whitespace-nowrap ${activeTab === 'utbk' ? 'active-tab' : 'text-secondary'}`}
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 animate-fade-in">
          {programsData[activeTab].map((item, index) => (
            <div key={index} className="bg-surface-container-lowest border border-outline-variant p-6 rounded-2xl flex flex-col hover:border-primary/50 hover:shadow-md transition-all shadow-sm group cursor-pointer">
              ...
              <div className="flex justify-between items-center mt-auto">
                ...
                <button className="bg-primary text-white px-4 py-2 rounded-xl text-sm font-bold hover:bg-primary-container transition-colors shadow-sm">
                  Pilih Program
                </button>
              </div>
            </div>
